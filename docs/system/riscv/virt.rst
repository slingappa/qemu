...
- rpmi=[on|off]
  ...
  CPPC exposes fixed virtual performance capability values and a fast-channel
  shared-memory region for guest-visible feedback counters. These values are
  synthetic and do not model host CPU frequency scaling. When
  ``aia=aplic-imsic`` is selected, ``virt`` also exposes the RPMI System MSI
  service. System MSI targets are accepted only inside the S-mode IMSIC MMIO
...
including System MSI through AIA IMSIC:
...
The OpenSBI banner should report RPMI-backed HSM, system suspend, and CPPC
support. After the guest reaches a shell, use guest-side RPMI/SBI probes to
...
normal guest reboot/poweroff commands to exercise RPMI system reset paths.
...
with ``riscv,rpmi-*`` compatible strings. Services that require an RPMI
channel identifier record that identifier in the corresponding service node.
