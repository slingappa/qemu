                              ...
                              const RiscvRpmiConfig *cfg)
{
    ...
    s->mm_store_path = g_strdup(cfg->mm_store_path);
}
...
static bool riscv_rpmi_mm_store_load(RiscvRpmiState *s, Error **errp)
{
    ...
    if (!g_file_test(s->mm_store_path, G_FILE_TEST_EXISTS)) {
        return true;
    }

    keyfile = g_key_file_new();
    if (!g_key_file_load_from_file(keyfile, s->mm_store_path,
                                   G_KEY_FILE_NONE, &gerr)) {
        error_setg(errp, "failed to load RPMI MM store '%s': %s",
                   s->mm_store_path, gerr->message);
        return false;
    }
...
static uint64_t riscv_rpmi_mm_store_save(RiscvRpmiState *s)
{
    ...
    if (!contents) {
        qemu_log_mask(LOG_GUEST_ERROR,
                      "failed to serialize RPMI MM store '%s': %s\n",
                      s->mm_store_path, gerr->message);
        return EFI_ACCESS_DENIED;
    }

    if (!g_file_set_contents(s->mm_store_path, contents, length, &gerr)) {
        qemu_log_mask(LOG_GUEST_ERROR,
                      "failed to save RPMI MM store '%s': %s\n",
                      s->mm_store_path, gerr->message);
        return EFI_ACCESS_DENIED;
    }
